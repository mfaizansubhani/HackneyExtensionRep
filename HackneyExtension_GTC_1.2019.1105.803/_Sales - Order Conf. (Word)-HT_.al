report 50001 "Sales - Order Conf. (Word)-HT"
{
  ...
  trigger OnAfterGetRecord()var ItemAttribute: Record "Item Attribute";
  ...
  trigger OnAfterGetRecord()var lItem: Record Item;
  ...
  begin
    ...
    if grecSalesLine.Find('-')then repeat "Line Amount"+=grecSalesLine."Line Amount";
        if Quantity = 0 then "Unit Price":=0
        else
    ...
    if Type = Type::Item then begin
      lItem.Get("No.");
      //<RD4269KNL>
      if lItemTrackingCode.Get(lItem."Item Tracking Code")then if(lItemTrackingCode."Variable Weight Tracking") and ((lItemTrackingCode."Lot Specific Tracking") or (lItemTrackingCode."Lot Sales Outbound Tracking") or (lItemTrackingCode."SN Specific Tracking") or (lItemTrackingCode."SN Sales Outbound Tracking"))then begin
  ...
  trigger OnAfterGetRecord()var lctxtFreight: Label 'Freight:';
  ...
  trigger OnAfterGetRecord()var CurrencyExchangeRate: Record "Currency Exchange Rate";
  ...
  begin
    ...
    if "Tax Area Code" = '' then begin
      ...
      Line.UpdateVATOnLines(0, "Sales Header", Line, VATAmountLine)end
    else
    ...
    if "No. Pallets" <> 0 then gtxtPallets:='No. Pallets:'
    else
      gtxtPallets:='';
    //</JF45105AC>
    //<JF50231AC>
  ...
  trigger OnPostReport()begin
    if LogInteraction and not IsReportInPreviewMode()then if "Sales Header".FindSet()then repeat "Sales Header".CalcFields("No. of Archived Versions");
          if "Sales Header"."Bill-to Contact No." <> '' then SegManagement.LogDocument(3, "Sales Header"."No.", "Sales Header"."Doc. No. Occurrence", "Sales Header"."No. of Archived Versions", DATABASE::Contact, "Sales Header"."Bill-to Contact No.", "Sales Header"."Salesperson Code", "Sales Header"."Campaign No.", "Sales Header"."Posting Description", "Sales Header"."Opportunity No.")
          else
  ...
  local procedure GetUOMText(UOMCode: Code[10]): Text[10]var UnitOfMeasure: Record "Unit of Measure";
  begin
    if not UnitOfMeasure.Get(UOMCode)then exit(UOMCode);
    exit(UnitOfMeasure.Description);
  end;
  local procedure CreateReportTotalLines()var TempSalesTaxAmountLine: Record "Sales Tax Amount Line" temporary;
  ...
  local procedure CreateUSReportTotalLines()begin
    ReportTotalsLine.DeleteAll();
    ReportTotalsLine.Add(SubtotalLbl, TotalSubTotal, true, false, false);
    ReportTotalsLine.Add(InvDiscountAmtLbl, TotalInvDiscAmount, false, false, false);
    ReportTotalsLine.Add(TotalTaxLbl, TotalAmountVAT, false, true, false);
  end;
  local procedure GetTaxSummarizedLines(var TempSalesTaxAmountLine: Record "Sales Tax Amount Line" temporary)var TaxArea: Record "Tax Area";
  ...
  begin
    ...
    if TaxArea."Use External Tax Engine" then SalesTaxCalculate.CallExternalTaxEngineForSales("Sales Header", true)
    else
  ...
  local procedure nsjjGetLotNo(): Text[30]var lrecItem: Record Item;
  ...
  begin
    ...
    if lrecReservEntry.Find('-')then begin
      ...
      repeat if lrecReservEntry."Lot No." <> '' then begin
          ...
          if(StrLen(gtxtLotNo[gintDetailLineNo]) + StrLen(lrecReservEntry."Lot No.") + StrLen(Format(-lrecReservEntry."Quantity (Base)")) + StrLen(Format(lrecItem."Base Unit of Measure")) + 6) < lintCountonEachLine then begin
            if lintCount = 0 then gtxtLotNo[gintDetailLineNo]:=lrecReservEntry."Lot No." + ' (' + Format(-lrecReservEntry."Quantity (Base)") + ' ' + Format(lrecItem."Base Unit of Measure") + ')'
            else
    ...
    if lrecTrackingSpecification.Find('-')then begin
      ...
      repeat if lrecTrackingSpecification."Lot No." <> '' then begin
          gtxtLotCaption:='Lot No.:';
          if(StrLen(gtxtLotNo[gintDetailLineNo]) + StrLen(lrecTrackingSpecification."Lot No.") + StrLen(Format(-lrecTrackingSpecification."Quantity (Base)")) + StrLen(Format(lrecItem."Base Unit of Measure")) + 6) < lintCountonEachLine then begin
            if lintCount = 0 then gtxtLotNo[gintDetailLineNo]:=lrecTrackingSpecification."Lot No." + ' (' + Format(-lrecTrackingSpecification."Quantity (Base)") + ' ' + Format(lrecItem."Base Unit of Measure") + ')'
            else
  ...
  var pdecLNWInvoiced: Decimal)var lcduCWMgt: Codeunit "Catch Weight Management";
